...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Caso 2: Si no se proporcionan `fecha` y `tourId`, devolver lista completa de guías
  ...
  try {
    ...
    const nuevoGuia = await prisma.guia.create({
      data: {
        nombre,
        foto,
        edad: edadInt, // Usar el valor convertido a número
        descripcion,
        celular,
        cedula,
      },
...
// Obtener fechas disponibles para un tour
export async function GET_FECHAS(req: Request) {
  const { searchParams } = new URL(req.url);
  const tourId = searchParams.get("tourId");
...
// Crear una reserva y asignar un guía automáticamente
export async function POST_RESERVA(req: Request) {
  try {
    const body = await req.json();
