...
export function useRequireAuth(requiredRole?: string) {
    ...
    useEffect(() => {
        ...
        if (requiredRole && session.user?.role !== requiredRole) {
            ...
            console.warn(`Usuario con rol ${session.user?.role} intentando acceder a recurso que requiere ${requiredRole}`);
            return;
        }
    }, [session, status, requiredRole]); // Remover router de las dependencias
